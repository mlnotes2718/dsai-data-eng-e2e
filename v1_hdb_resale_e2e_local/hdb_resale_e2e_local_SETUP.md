# DSAI - HDB Resale Price End to End Pipeline - Local Development
      ...
      plugins:
        loaders:
          - name: target-bigquery
            config:
              dataset_id: dev_postgres_hdb_resale_raw
    ...
    config:
      plugins:
        loaders:
          - name: target-bigquery
            config:
              dataset_id: prod_postgres_hdb_resale_raw
...
```bash
meltano run --environment=dev tap-postgres target-bigquery
```
